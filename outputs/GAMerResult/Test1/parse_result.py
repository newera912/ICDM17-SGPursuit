...
def main():
    output=open("../../result/VaryingNumOfAttributes-GAMer.txt","w")
    for attNum in [20,40,80,100]:
        file_name = 'VaryingNumOfAttributes_APDM_Dense_subgraph_in_0.35_out_0.1_numClusters_10_TrueSGSize_30_FeasNum_'+str(attNum)+'_trueFeasNum_10_sigmas1_0.0316.txt'
        data = dict()
...
def get_single_file_result(num_):
    file_name = 'VaryingNumOfAttributes_APDM_Dense_subgraph_in_0.35_out_0.1_Cluster_10_TrueSGSize_30_FeaNum_'+str(num_)+'_trueFeasNum_10_.txt'
    data = dict()
    with open(file_name) as f:
        for each_line in f.readlines():
            items = [float(each_item) for each_item in each_line.rstrip().split(" ")]
