...
public class DenseClusterDetection {
 ...
 int k, int s,
	 ...
	 * R^{|num_nodes|X(num_feature+1)}, where 1st columns are node IDs
	 * edgeList: int[|num_edge|][2]. edgeList is a list of edges,
	 * edgeList=[..,[start_nodeID,end_nodeID],...]
	 * s: int. maximum number feature number. The maximum number of |y|<s
	 * k: int. the total sparsity of x
	 * lambda: density teem threshold parameter, deafult lambda=5.0
	 * numCluster: number of clusters need to be returned
	 * 
	 * 
	 * Output:
	 * output final results store into result file. Format:
	 * each line represents a cluster, we used "|" separate the detected subset
	 * of nodes and corresponding subset of features.
	 * for example:
