...
public class MultiClusterSGPursuit {
	...
	private static void MultiClusterDetectionOn3Measure(String fileName,
		...
		for (int t : c_num) {
			...
			double average_score=round((avg_clusterSize+avg_density+avg_euclidean)/3.0,2);
	...
	public static void experiments_RealdataSets() {
		...
		for (String dataset : new String[] { "dblp" }) {
			String fileName = dataset + "_APDM.txt";
			long startTimeAll = System.nanoTime();
			System.out.println(fileName);
			MultiClusterDetectionOn3Measure(root + fileName, "", 5, 3, lambda,
					5, dataset);
	...
	public static void experiments_RealdataSets3Measure() {
		for (String dataset : new String[] { "dblp" }) { // "dblp", "genes",
															// "imdb",
															// "dfb","arxivSmall"
			log(">>>" + LocalDateTime.now() + " ", dataset);
			for (double lambda : new double[] { 5.0 }) {
				for (int s : new int[] { 3, 5, 10, 15, 20 }) {
					for (int k : new int[] { 5, 10, 15, 30 }) {
						...
						String root = "data/DenseGraph/Realdata/apdm/";
						String outroot = "outputs/GAMerResult/";
						List<Integer> c_num = (List<Integer>) Arrays.asList(20);
						// "dblp", "genes", "imdb", "dfb","arxivSmall",
						// "patents", "arxivLarge"

						log(" k=" + k + " s=" + s + " lambda=" + lambda,
								dataset);
						String fileName = dataset + "_norm_APDM.txt";
						long startTimeAll = System.nanoTime();
						log(fileName, dataset);
						for (int c : c_num) {
							MultiClusterDetectionOn3Measure(root + fileName,
									"", k, s, lambda, c, dataset);
						}
						log("\n Total running time: "
								+ ((System.nanoTime() - startTimeAll) / 1e9)
								+ "\n",
