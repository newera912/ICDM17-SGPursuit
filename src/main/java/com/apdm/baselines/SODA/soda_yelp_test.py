...
def outlierScore(neighbors, W, p, edges, C, s):
    ...
    tempList.append(Hm)
    tempList.append(Lm)
    outlog.write("Hm: "+str(len(Hm))+" sum="+str(np.sum(Hm)) +"\n")
    outlog.write("Lm: "+str(len(Lm))+" sum="+str(np.sum(Lm)) +"\n")    
    outlog.write("w: "+str(len(w))+" sum="+str(np.sum(w)) +"\n")
    outlog.write("zeta: : "+str(len(zeta))+" sum="+str(np.sum(zeta)) +"\n")
    for i in w:
        tempList.append(i)
    for i in zeta:
        tempList.append(i)
    outlog.write(str(len(tempList))+" "+str(np.sum(tempList)) +"\n")
    outlog.write(str(len(b))+" "+str(np.sum(b)) +"\n")   
    outlog.write(str(len(c))+" "+str(np.sum(c)) +"\n") 
    outlog.flush()         
    ...
    # print b
    # print c
    solvers.options['glpk'] = {'maxiters':25}
...
def soda_scan(V, edges, W, radius, s, C, true_nodes, g):
    ...
    for i,S in enumerate(S_data):
        ...
        print('_____________________________ {}-{}_________________________________'.format(i,radius))
        if len(neighborhoods) < 100:  
            
            #outlog.write(str(i)+" S_size="+str(len(S))+" neighborSize="+str(len(neighborhoods))+" S=["+" ".join(map(str,S)) +"]\n")
            outlog.write(str(i)+" S_size="+str(len(S))+" neighborSize="+str(len(neighborhoods)) +"\n")
            outlog.flush()
...
#                 outlog.flush()          
            ...
            outlog.write("time= "+str(round(time.time()-t0,2))+"\n")
            outlog.flush()
            if fvalue != None:
                scores.append([fvalue, S, feature_set])
                print(fvalue, S)
                print("Yes,Have Solution....\n\n")
                outlog.write("Yes,Have Solution....\n\n")
                outlog.flush()
                #    sorted(scores, key=lambda x:x[0], reverse=True)
            else:
                print("No,Solution....\n\n")
                outlog.write("No,Solution....\n\n")
                outlog.flush()
    #print(scores)
    if len(scores)==0:
        return 0.0,[],[]
...
def test_on_yelp_data():
    file_path = '/home/apdm02/Dropbox/expriments/Yelp/graph/'
    ...
    for each_file in file_list: 
        ...
        fp = open('./final_result_SODA_yelp_different_para_New_May19.txt', 'a')
        ...
        for ridus in [2, 3, 4, 5]:
            ...
            if len(S)<1:
                print('\n\n>>>>>>>>>{} {}<<<<<<<<<<<<<\n\n'.format(each_file.split("_")[-1],ridus))
                outlog.write('\n\n>>>>>>>>>{} {}<<<<<<<<<<<<<\n\n'.format(each_file.split("_")[-1],ridus))
                outlog.flush() 
                continue
            print(S, R)
