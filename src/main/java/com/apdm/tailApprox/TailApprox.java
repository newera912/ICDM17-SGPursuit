...
import com.apdm.base.DisjointSet;
import com.apdm.fastPCST.FastPCST;
import com.apdm.headApprox.Digraph;
import com.apdm.headApprox.DirectedEulerianCycle;
...
public class TailApprox {
     ...
     *
     * @param edges       // Graph G
     * @param edgeCostsc  // edge Cost c
     * @param prizesPi    // node prizes Pi
     * @param g           // number of connected components g
     * @param costBudgetC // cost Budget C
     * @param nu          // constant parameter nu
     * @param delta       // constant parameter delta
     */
    public TailApprox(ArrayList<Integer[]> edges, ArrayList<Double> edgeCostsc, ArrayList<Double> prizesPi, int g,
                      double costBudgetC, double nu, double delta, boolean debug) {
    ...
    public TailApprox(ArrayList<Integer[]> edges, ArrayList<Double> edgeCostsW, double[] z, int s, int g, double B, boolean debug) {
        this(edges, edgeCostsW, z, s, g, B, null, debug);
    }
    ...
    public TailApprox(ArrayList<Integer[]> edges, ArrayList<Double> edgeCostsW, double[] z, int s, int g, double B,
                      int[] trueSubGraph, boolean debug) {
    ...
    private F run() {
        ...
        if (debug == true) {
            System.out.println(getCostsLambda(lambda0));
        }
        ...
        if (verboseLevel > 0) {
            if (debug == true) {
                ...
                System.out.println("forest nodes: " + forest.nodesInF);
                System.out.println("forest nodes size: " + forest.nodesInF.size());
            }
        }
        /** check this further, == 0.0D */
        if (verboseLevel > 0) {
            if (debug == true) {
                System.out.println("costF: " + forest.costF + " 2*C: " + 2.0D * C + " piFBar: " + forest.piFBar);
            }
        }
        ...
        while (true) {
            ...
            if (changed_R && changed_L) {
                ...
                lambdaR = 0.0D;
                lambdaL = 2.0D * lambdaLUnit;
                lambdaLUnit = lambdaL;
            }
        }

//        System.out.println(getCostsLambda(lambdaL));
//        System.out.println("[ L R ]: " + lambdaL + " " + lambdaR);
//        System.out.println("c(F): " + forest.costF);
//        System.out.println("c(F) <= 2*nu*B" + forest.costF + " " + nu * C);
